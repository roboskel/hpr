...
def offline_train():
    ...
    global fr_index , ind ,all_hogs, all_surf,fig1,ax,kat,wall_cart,wall,kat
    global z, zscale, slot_count, human_l ,cluster_l,slot_touched,all_scans
    global timewindow ,slot_data , phi,wall_index,annotations,em_index
    ...
    mat=sio.loadmat('mat_files/cinema1.mat')
...
def cluster_train(clear_data):
    ...
    for k in range(1,max_label+1) :
        ...
        if len(filter[0])>40 :
            ...
            ax.scatter(xi[filter],yi[filter], zi[filter], 'z', 30,c=cc[k-1])
            fig1.add_axes(ax)
